...
function Level:update(dt)
    ...
    if self.launchMarker.launched then
        ...
        if not self.launchMarker.alienSpawned and not self.launchMarker.collided then
            if love.keyboard.wasPressed('space') then
                self.launchMarker:spawnNewAliens()
            end
        end
        ...
        for i, alien in pairs(self.launchMarker.aliens) do
            ...
            if xPos < 0 or xPos > VIRTUAL_WIDTH * 2 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
